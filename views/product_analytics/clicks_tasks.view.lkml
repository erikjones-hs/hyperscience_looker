  ...
  dimension: transcription_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_transcription' then 1 else 0 ;;
  }

  dimension: flex_extract_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_flexible_extraction' then 1 else 0 ;;
  }

  dimension: document_classification_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_document_classification' then 1 else 0 ;;
  }

  dimension: identification_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_identification' then 1 else 0 ;;
  }

  dimension: qa_document_classification_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_qa_document_classification' then 1 else 0 ;;
  }

  dimension: qa_transcription_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_qa_transcription' then 1 else 0 ;;
  }

  dimension: qa_identification_fl {
    type: number
    sql: CASE WHEN ${product_event} = 'perform_tasks_qa_identification' then 1 else 0 ;;
  }
